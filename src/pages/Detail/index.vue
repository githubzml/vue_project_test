            ...
            <div class="chooseArea">
              ...
              <dl v-for="saleAttr in spuSaleAttrList" :key="saleAttr.id">
                ...
                <dd
                  ...
                  @click="
                    handlerActive(saleAttrValue, saleAttr.spuSaleAttrValueList)
                  "
                >
...
export default {
  ...
  computed: {
    ...mapGetters("adetail", ["categoryView", "skuInfo", "spuSaleAttrList"]),
  },
  mounted() {
    console.log("this", this.$route.params);

    // this.$store.dispatch("adetail/getGoodDetailInfo", this.id);

    this.$store.dispatch("adetail/getGoodDetailInfo", this.$route.params.id);
  },
  watch: {
    count(newValue) {
  ...
  methods: {
    // 1. 用来切换小图实现大图显示并让小图高亮
    handlerActive(current, parentAttr) {
      // 在这里面要实现排它思路，单击哪个哪个才高亮也就是才拥有active类
      // 但是active类又是受当前对象中的isChecked的值来控制的，只有为1的时候才应该添加Active
      // 因此没有被单击过的isChecked都应该是0 单击的才是1
      parentAttr.forEach((item) => {
        item.isChecked = "0"; // 其它所有的同级兄弟元素的isChecked全部设置为0
      });

      current.isChecked = "1"; // 自己的这个设置为1
    },
    // 2. 跳转到添加购物车成功页面
    async addSku2Cart() {
      // 合法校验
    ...
    async addShopCar() {
      try {
        await this.$store.dispatch("adetail/addOrUpdateShopCart", {
